...
#        "vegetation__cover_fraction": {    ERKNAI C/O
#            "dtype": float,
#            "intent": "in",
#            "optional": False,
#            "units": "None",
...
#        },
    ...
    def initialize(
        ...
        self._LAI_max = np.choose( ### ERKAN-- IS THERE ANY LOGIC OF REPEATING THESE
            self._vegtype,
            [
                LAI_max_grass,
                LAI_max_shrub,
    ...
    def update(self):
        ...
        ksat = self._cell_values["saturated__hydraulic_conductivity"]
        #ksat_t = self._cell_values["saturated__hydraulic_conductivity"]  ---ERKANI C/O
        #ksat_pre = self._cell_values["saturated__hydraulic_conductivity_pre"] ---ERKANI C/O
        #lai_t = self._cell_values["live_leaf_area_index"]        --ERKANI C/O
        #lai_pre = self._cell_values["live_leaf_area_index_pre"]   -- ERKAN C/O
        ...
        #self._vegcover = self._cell_values["vegetation__cover_fraction"]  #ERKAN---- WE DON'T NEED VEG COVER, THIS CAN BE CALCULATED INTERNALLY FROM LAI AS NEEDED SEE THE CALCULATION OF
                                                                                #self._fr
        ...
        for cell in range(0, self._grid.number_of_cells):
            ...
            ksat_cell= ksat[cell] 
           # ksat_t_cell = ksat_t[cell]   ---ERKANI C/O
           # ksat_cell_pre = ksat_pre[cell]
           # lai_t_cell = lai_t[cell]
           # lai_cell_pre = lai_pre[cell]
            # print cell
            s = self._SO[cell]
            fbare = self._fbare
            ZR = self._zr[cell] 
            ...
            #Int_cap = Int_cap * (lai_t_cell/lai_cell_pre)  ---ERKANI C/O
            ...
            self._ETmax[cell] = Ep
            nu = max(((Ep / 24.0) / 1000.0) / (pc * ZR), 1e-12)  # Loss function parameter
            nuw = ((self._soil_Ew / 24.0) / 1000.0) / (pc * ZR)
            # Loss function parameter
            sini = self._SO[cell] + ((Peff + self._runon) / (pc * ZR * 1000.0))

            if sini > 1.0:
                R_sat = ((sini - 1.0) * pc * ZR * 1000.0) 
                # print 'Runoff =', self._runoff
                sini = 1.0
            else:
                R_sat = 0.0

            self._runoff[cell] = Rs + R_sat

            if sini >= fc:
                tfc = (1.0 / (beta * (mu - nu))) * (
                    beta * (fc - sini)
                    + np.log(np.maximum((nu - mu + mu * np.exp(beta * (sini - fc))) / nu, 1e-12))
                )
                tsc = ((fc - sc) / nu) + tfc
